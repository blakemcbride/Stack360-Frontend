...
window.popup_startup = function (inData) {
    return new Promise(function (resolve, reject) {
        ...
        const WS = "com.arahant.services.standard.components.orgGroupSearch";
        ...
        {
            const columnDefs = [
                {headerName: 'Org Group', field: 'name', width: 75  }
            ];
        ...
        function search() {
            ...
            grid.clear();
            Server.call(WS, "OrgGroupSearch", data).then(function (res) {
                if (res._Success) {
                    grid.addRecords(res.orgGroups);
                    grid.clearSelection();
                    if (res.isMore)
                        $$('cm-cs-msg').setColor("red").setValue("Showing " + res.orgGroups.length + " (more)")
                    else
                        $$('cm-cs-msg').setColor("black").setValue("Showing " + res.orgGroups.length)
                }
            });
        }
