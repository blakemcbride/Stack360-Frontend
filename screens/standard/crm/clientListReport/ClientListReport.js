...
(async function () {
    ...
    $$('btn-report').onclick(() => {
        const data = {
            ...
            contractDate: $$('chk-contract-date').getValue(),
            billingRate: $$('chk-billing-rate').getValue(),
            sortAsc: $$('sort-direction').getValue(),
            sortType: $$('sort-column').getValue(),
        }
        AWS.callSoap(WS, 'getClientReport', data).then(ret => {
            if (ret.wsStatus === '0') {
