...
(async function () {
    ...
    const WS = 'StandardCrmProspectSource';
    ...
    function updateGrid() {
        ...
        AWS.callSoap(WS, "listProspectSources").then(function (res) {
            if (res.wsStatus === '0') {
                ...
                $$('status').setValue('Displaying ' + res.item.length + ' Prospect Sources');
            }
        });
    }
    ...
    $$('add').onclick(() => {
        $$('dp-title').setValue('Add Prospect Source');
        reset();
        ...
        $$('dp-ok').onclick(() => {
            ...
            AWS.callSoap(WS, "newProspectSource", data).then(function (res) {
                if (res.wsStatus === '0') {
                    updateGrid();
                    Utils.popup_close();
                }
    ...
    function edit() {
        ...
        $$('dp-title').setValue('Edit Prospect Source');
        ...
        $$('dp-ok').onclick(() => {
            ...
            AWS.callSoap(WS, "saveProspectSource", data).then(function (res) {
                if (res.wsStatus === '0') {
                    updateGrid();
                    Utils.popup_close();
                }
    ...
    $$('delete').onclick(() => {
        Utils.yesNo('Confirmation', 'Are you sure you wish to delete the selected Prospect Source?', () => {
            const data = {
                ids: [grid.getSelectedRow().id]
            };
            
            AWS.callSoap(WS, "deleteProspectSources", data).then(function (res) {
                if (res.wsStatus === '0') {
                    updateGrid();
                }
            });
