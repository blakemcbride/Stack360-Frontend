...
window.ProspectOrgGroup = {};

(async function () {

    const WS = 'StandardCrmProspectOrgGroup';
    ...
    ProspectOrgGroup.goBack = function (lvl) {
        while (lvl--) {
            screenStack.pop();
            nameStack.pop();
        }
    ...
    function updateBreadCrumb() {
        ...
        let bc = '<div style="display: flex; flex-direction: row;align-items: center"><a class="" onclick="ProspectOrgGroup.goBack(' + (levels-1) + ')" style="font-weight: bold; cursor:pointer;">Top</a>';
        let i = 1;

        for (; i < levels; i++)
            bc += separator + '<a class="" onclick="ProspectOrgGroup.goBack(' + (levels - (i + 1)) + ')" style="cursor:pointer;color:black;font-weight: bold;">' + nameStack[i] + '</a>';
    ...
    let orgGroupGrid = new AGGrid('org-groups-grid', [
        {headerName: 'Prospect Organizational Unit Name', field: 'name', width: 55 },
    ...
    let employeesGrid = new AGGrid('employees-grid', [
        {headerName: 'Last Name', field: 'lname', width: 70 },
        {headerName: 'First Name', field: 'fname', width: 70 },
        {headerName: 'Primary', field: 'primaryIndicator', width: 50 },
    ], 'personId');
    employeesGrid.show();
    ...
    function updateEmployeesGrid() {
        ...
        AWS.callSoap(WS, "listProspectContacts", data).then(function (res) {
            if (res.wsStatus === '0') {
                res.persons = Utils.assureArray(res.persons);
                employeesGrid.addRecords(res.persons);
                $$('employees-status').setValue(`Displaying ${res.persons.length} Contact` + (res.persons.length > 1 ? 's' : ''));
            }
        });
    ...
    const editOrgGroupPopup = () => {
        ...
        $$('ogp-title').setValue('Edit Prospect Organizational Group');
    ...
    $$('org-group-delete').onclick(() => {
        ...
        if (row.type !== 'Group') {
            Utils.showMessage('Error', 'Only Prospect Organizational Groups can be deleted from this list.');
            return;
        } else {
            Utils.yesNo('Confirmation', 'Are you sure you wish to delete the selected Prospect Organizational Group?', () => {
                const data = {
    ...
    const searchGroupPopup = () => {
        ...
        let reset = () => {
            ...
            $$('sgp-count').setValue(`Displaying 0 Prospect Organizational Units`).setColor('black');
        };
        ...
        let initDataGrid = () => {
            // Setup data grid.
            let columnDefs = [
                {headerName: 'Prospect Organizational Unit Name', field: 'name', width: 75},
        ...
        $$('sgp-search').onclick(() => {
            ...
            AWS.callSoap(WS, 'searchOrgGroupsGeneric', inParams).then(data => {
                if (data.orgGroups) {
                    ...
                    if (count >= data.cap) {
                        $$('sgp-count').setValue(`Displaying ${count} Prospect Organizational Unit` + (count > 1 ? 's' : '') + (' (Limit)')).setColor('red');
                    } else {
                        $$('sgp-count').setValue(`Displaying ${count} Prospect Organizational Unit` + (count > 1 ? 's' : '')).setColor('black');
                    }
                } else {
                    $$('sgp-count').setValue(`Displaying 0 Prospect Organizational Units`).setColor('black');
                }
    ...
    const associateGroup = () => {
        ...
        let reset = () => {
            ...
            formSearchGrid.clear();
            $$('agp-count').setValue(`Displaying 0 Prospect Organizational Units`).setColor('black');
        };
        ...
        $$('agp-search').onclick(() => {
            ...
            AWS.callSoap(WS, 'searchOrgGroups', inParams).then(data => {
                if (data.orgGroups) {
                    ...
                    let count = Utils.assureArray(data.orgGroups).length;
                    if (count >= data.cap) {
                        $$('agp-count').setValue(`Displaying ${count} Prospect Organizational Unit` + (count > 1 ? 's' : '') + (' (Limit)')).setColor('red');
                    } else {
                        $$('agp-count').setValue(`Displaying ${count} Prospect Organizational Unit` + (count > 1 ? 's' : '')).setColor('black');
                    }
                } else {
                    $$('agp-count').setValue(`Displaying 0 Prospect Organizational Units`).setColor('black');
                }
    ...
    $$('org-group-disassociate').onclick(() => {
        Utils.yesNo('Confirmation', 'Are you sure you wish to disassociate the selected Prospect Organizational Group?', () => {
            const data = {
    ...
    const addEmployeePopup = () => {
        ...
        Utils.popup_open('employee-popup', 'ep-fname');
        ...
        const ok = () => {
            ...
            const data = {
                ...
                type: $$('ep-type').getValue(),
                workFax: $$('ep-fax').getValue(),
                workPhone: $$('ep-phone').getValue()
            };

            AWS.callSoap(WS, 'newProspectContact', data).then(res => {
                if (res.wsStatus === '0') {
                    updateEmployeesGrid();
                }
            })
    ...
    $$('employee-add').onclick(addEmployeePopup);

    const editEmployeePopup = () => {
        let parentGroupId = screenStack[screenStack.length-1];
        $$('ep-title').setValue('Edit Contact');
        $$('ep-fname').clear();
        $$('ep-job-title').clear();
        $$('ep-lname').clear();
        $$('ep-phone').clear();
        ...
        AWS.callSoap(WS, 'loadProspectContact', data).then(res => {
            if (res.wsStatus === '0') {
        ...
        const ok = () => {
            ...
            AWS.callSoap(WS, 'saveProspectContact', data).then(res => {
                if (res.wsStatus === '0') {
                    updateEmployeesGrid();
                }
            })
    ...
    $$('employee-edit').onclick(editEmployeePopup);
    employeesGrid.setOnRowDoubleClicked(editEmployeePopup);

    $$('employee-delete').onclick(() => {
        const row = employeesGrid.getSelectedRow();
        ...
        Utils.yesNo('Confirmation', 'Are you sure you want to delete the selected Prospect Contact?', () => {
            ...
            AWS.callSoap(WS, "deleteProspectContact", data).then(function (res) {
                if (res.wsStatus === '0') {
                    updateEmployeesGrid();
                }
            });            
    ...
    const associateContact = () => {
        ...
        let reset = () => {
            ...
            formSearchGrid.clear();
            $$('aep-count').setValue(`Displaying 0 Contacts`).setColor('black');
        };
        ...
        $$('aep-search').onclick(() => {
            ...
            AWS.callSoap(WS, 'searchProspectContacts', inParams).then(data => {
                if (data.contacts) {
                    ...
                    if (count >= data.cap) {
                        $$('aep-count').setValue(`Displaying ${count} Contact` + (count > 1 ? 's' : '') + (' (Limit)')).setColor('red');
                    } else {
                        $$('aep-count').setValue(`Displaying ${count} Contact` + (count > 1 ? 's' : '')).setColor('black');
                    }
                } else {
                    $$('aep-count').setValue(`Displaying 0 Contact`).setColor('black');
                }
            });
        });
    ...
    $$('employee-associate').onclick(associateContact);

    $$('employee-disassociate').onclick(() => {
        Utils.yesNo('Confirmation', 'Are you sure you want to disassociate the selected Prospect Contact?', () => {
            const data = {
