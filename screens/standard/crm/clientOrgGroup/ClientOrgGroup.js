...
window.ClientOrgGroup = {};

(async function () {

    const WS = 'StandardCrmClientOrgGroup';
    ...
    ClientOrgGroup.goBack = function (lvl) {
        while (lvl--) {
            screenStack.pop();
            nameStack.pop();
        }
    ...
    function updateBreadCrumb() {
        ...
        let bc = '<div style="display: flex; flex-direction: row;align-items: center"><a class="" onclick="ClientOrgGroup.goBack(' + (levels-1) + ')" style="font-weight: bold; cursor:pointer;">Top</a>';
        let i = 1;

        for (; i < levels; i++)
            bc += separator + '<a class="" onclick="ClientOrgGroup.goBack(' + (levels - (i + 1)) + ')" style="cursor:pointer;color:black;font-weight: bold;">' + nameStack[i] + '</a>';
    ...
    let orgGroupGrid = new AGGrid('org-groups-grid', [
        {headerName: 'Client Organizational Unit Name', field: 'name', width: 55 },
    ...
    function updateContactsGrid() {
        ...
        AWS.callSoap(WS, "listClientContacts", data).then(function (res) {
            if (res.wsStatus === '0') {
    ...
    const addOrgGroupPopup = () => {
        ...
        $$('ogp-title').setValue('Add Client Organizational Group');
        $$('ogp-name').clear();
        $$('ogp-id').clear();
    ...
    const editOrgGroupPopup = () => {
        ...
        $$('ogp-title').setValue('Edit Client Organizational Group');
    ...
    $$('org-group-delete').onclick(() => {
        ...
        if (row.type !== 'Group') {
            Utils.showMessage('Error', 'Only Client Organizational Groups can be deleted from this list.');
            return;
        } else {
            Utils.yesNo('Confirmation', 'Are you sure you wish to delete the selected Client Organizational Group?', () => {
                const data = {
    ...
    const searchGroupPopup = () => {
        ...
        let reset = () => {
            ...
            $$('sgp-count').setValue(`Displaying 0 Client Organizational Units`).setColor('black');
        };
        ...
        let initDataGrid = () => {
            // Setup data grid.
            let columnDefs = [
                {headerName: 'Client Organizational Unit Name', field: 'name', width: 75},
        ...
        $$('sgp-search').onclick(() => {
            ...
            AWS.callSoap(WS, 'searchOrgGroupsGeneric', inParams).then(data => {
                if (data.orgGroups) {
                    ...
                    if (count >= data.cap) {
                        $$('sgp-count').setValue(`Displaying ${count} Client Organizational Unit` + (count > 1 ? 's' : '') + (' (Limit)')).setColor('red');
                    } else {
                        $$('sgp-count').setValue(`Displaying ${count} Client Organizational Unit` + (count > 1 ? 's' : '')).setColor('black');
                    }
                } else {
                    $$('sgp-count').setValue(`Displaying 0 Client Organizational Units`).setColor('black');
                }
    ...
    const associateGroup = () => {
        const parentOrgGroupId = screenStack[screenStack.length-1];
        ...
        const reset = () => {
            ...
            formSearchGrid.clear();
            $$('agp-count').setValue(`Displaying 0 Client Organizational Units`).setColor('black');
        };

        const ok = () => {
            const data = {
                parentGroupID: parentOrgGroupId,
            ...
            AWS.callSoap(WS, 'addGroupToGroup', data).then(res => {
                if (res.wsStatus === '0') {
                    reset();
                    updateOrgGroupGrid(parentOrgGroupId, nameStack[nameStack.length-1]);
                    Utils.popup_close();
                }
            });
        };
        ...
        $$('agp-search').onclick(() => {
            const inParams = {
                ...
                orgGroupId: screenStack[1]  // owning company
            };
            ...
            AWS.callSoap(WS, 'searchOrgGroups', inParams).then(data => {
                if (data.orgGroups) {
                    const records = Utils.assureArray(data.orgGroups);
                    formSearchGrid.addRecords(records);
    
                    let count = Utils.assureArray(data.orgGroups).length;
                    if (count >= data.cap) {
                        $$('agp-count').setValue(`Displaying ${count} Client Organizational Unit` + (count > 1 ? 's' : '') + (' (Limit)')).setColor('red');
                    } else {
                        $$('agp-count').setValue(`Displaying ${count} Client Organizational Unit` + (count > 1 ? 's' : '')).setColor('black');
                    }
                } else {
                    $$('agp-count').setValue(`Displaying 0 Client Organizational Units`).setColor('black');
                }
    ...
    $$('org-group-disassociate').onclick(() => {
        Utils.yesNo('Confirmation', 'Are you sure you wish to disassociate the selected Client Organizational Group?', () => {
            const data = {
    ...
    const addEmployeePopup = () => {
        ...
        const ok = () => {
            ...
            const data = {
                ...
                contactType: Number($$('ep-type').getValue())
            };

            AWS.callSoap(WS, 'newClientContact', data).then(res => {
                if (res.wsStatus === '0') {
                    updateContactsGrid();
                }
            })
    ...
    const editEmployeePopup = () => {
        ...
        AWS.callSoap(WS, 'loadClientContact', data).then(res => {
            ...
            $$('ep-type').setValue(res.contactType);
        ...
        const ok = () => {
            ...
            AWS.callSoap(WS, 'saveClientContact', data).then(res => {
                if (res.wsStatus === '0') {
                    updateContactsGrid();
                }
            })
    ...
    $$('contact-delete').onclick(() => {
        ...
        Utils.yesNo('Confirmation', 'Are you sure you want to delete the selected Client Contact?', () => {
            const data = {
                ids: row.personId
            };
    ...
    const associateContact = () => {
        ...
        $$('aep-search').onclick(() => {
            let inParams = {
                ...
                orgGroupId: parentOrgGroupId
            };

            // Clear the grid.
            formSearchGrid.clear();

            AWS.callSoap(WS, 'searchClientContacts', inParams).then(data => {
                if (data.contacts) {
    ...
    $$('contact-disassociate').onclick(() => {
        Utils.yesNo('Confirmation', 'Are you sure you want to disassociate the selected Client Contact?', () => {
            const data = {
