...
(async function () {
    ...
    const SWS = 'StandardProjectMessage';
    const RWS = "com.arahant.services.standard.project.message";
    ...
    const sendFilterParms = {
        personType: 'receiver',
        senderId: Framework.userInfo.personId,
        receiverId: null,
        fromDate: 0,
        fromDateIndicator: 0,
        toDate: 0,
...
*/
    ...
    async function getAllWorkers() {
        ...
        allWorkers = Utils.assureArray(data.people);
    }
    ...
    async function getShifts(projectId) {
        ...
        const res = await Server.call(RWS, 'GetShifts', data);
    ...
    function searchReceiver(recipients, type, limitToOne) {
        return new Promise(function (resolve, reject) {
            ...
            const srInitDataGrid = () => {
                ...
                searchReceiverGrid.multiSelect();
                searchReceiverGrid.show();
                $$('receiverAddAll').disable();
            };
            ...
            function addRecipient() {
                ...
                $$('receiverAddAll').enable(searchReceiverGrid.getNumberOfRows() > 0);
            }

            $$('receiverAddAll').onclick(() => {
                const nodes = searchReceiverGrid.getAllRows();
                for (let i = 0; i < nodes.length; i++) {
                    const selectedNode = nodes[i];
                    const obj = {
                        personId: selectedNode.personID,
    ...
    async function viewMessage() {
        ...
        function find(person) {
            for (let i=0 ; i < allWorkers.length; i++) {
                if (allWorkers[i].personId === person.personId)
                    return 1;
            }
            return 0;
        }
        ...
        for (let i = 0; i < data.toPersons.length; i++) {
            let person = data.toPersons[i];
            nFound += find(person);
            if (names)
                names += "; ";
            names += person.displayName;
        }
        if (nFound && nFound === allWorkers.length)
            $$('messageReceiver').setValue("All assigned");
        else
        ...
        if (nFound && nFound === allWorkers.length)
            $$('messageCC').setValue("All assigned");
        else
        ...
        if (nFound && nFound === allWorkers.length)
            $$('messageBCC').setValue("All assigned");
        else
