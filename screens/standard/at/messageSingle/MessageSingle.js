...
(function () {
    ...
    const SWS = 'StandardAtMessageSingle';
    const RWS = "com.arahant.services.standard.at.messageSingle";
    const applicantId = Utils.getData(HR_PERSON_ID);
    const applicantName = Utils.getData(HR_PERSON_NAME);
    const applicantPhone = Utils.getData(HR_PERSON_PHONE);
    const applicantEmail = Utils.getData(HR_PERSON_EMAIL);

    $$('worker').setValue(applicantName);
...
*/
    ...
    async function newMessage(updateGridFlg, recipients) {
        ...
        if (recipients && recipients.length) {
            let r = recipients[0];
            let name = r.personName;
            if (r.emailAddress && r.mobilePhone)
                name += ' (' + r.emailAddress + ', ' + r.mobilePhone + ')';
            else if (r.emailAddress)
                name += ' (' + r.emailAddress + ')';
            else if (r.mobilePhone)
                name += ' (' + r.mobilePhone + ')';
            $$('messageReceiver').setValue(name);
        }
            ...
            if ($$('messageEmailCB').getValue()) {
                for (let i=0; i < recipients.length; i++)
                    if (!recipients[i].emailAddress) {
                        Utils.showMessage('Error', 'Recipient does not have an email address.');
                        return false;
                    }
            ...
            if ($$('messageTextCB').getValue()) {
                for (let i=0; i < recipients.length; i++)
                    if (!recipients[i].mobilePhone) {
                        Utils.showMessage('Error', 'Recipient does not have a mobile phone number.');
                        return false;
                    }
