...
(function () {
    ...
    const SWS = 'StandardHrMessage';
    const RWS = "com.arahant.services.standard.hr.message";
...
*/
    ...
    $$('newSent').onclick(() => {
        Server.call(RWS, 'GetPersonInfo', { personId: personId }).then(function (data) {
            if (data._Success) {
                sentNewMessage([{
                    personId: personId,
                    personName: personName,
                    mobilePhone: data.info.phone_number,
                    emailAddress: data.info.personal_email
                }]);
            }
        })
    });
